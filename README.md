# This fork adds display filtering by y-coordinate.
Этот мод от авторов jenchanws и enbyv с моей небольшой доработкой.
Я добавил поля диапазона высоты y min и y max. Они позволят отображать элементы, чья (средняя) высота входит в диапазон.

Фильтр срабатывает только после обновления состава элементов (для того, чтобы обновить пути - поставьте, например рельсу в мире, для поездов - передвиньте поезд и так далее)

Эта функция была нужна мне для того, чтобы не возникало путаницы между несвязанными путями, расположенными на разных высотах.
![img.png](img.png)
